...
export function Label(props: LabelProps) {
    return <RACLabel {...props} className={twMerge('font-sans text-sm text-neutral-600  font-medium cursor-default w-fit', props.className)} />;
}
...
export function Input(props: InputProps) {
    return <RACInput {...props} className={composeTailwindRenderProps(props.className, 'px-3 py-0 min-h-9 flex-1 min-w-0 border-0 outline outline-0 bg-white font-sans text-sm text-neutral-800  placeholder:text-neutral-600  disabled:text-neutral-200 disabled:placeholder:text-neutral-200 [-webkit-tap-highlight-color:transparent]')} />
}
